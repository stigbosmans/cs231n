...
def affine_forward(x, w, b):
    ...
    out = x.reshape(N, D).dot(w) + b.reshape(1, w.shape[1])
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    cache = (x, w, b)
...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        cache['x'] = x
        cache['mean'] = mean
        cache['xminmean'] = xminmean
        cache['var'] = var
        cache['varsq'] = varsq
        cache['divvarsq'] = divvarsq
        cache['xhat'] = xhat
        cache['gamma'] = gamma
        cache['beta'] = beta
        cache['eps'] = eps
        ...
        #######################################################################
        normalizedx = (x - running_mean) / np.sqrt(running_var + eps)
        out = gamma * normalizedx + beta
        #######################################################################
        #                          END OF YOUR CODE                           #
        #######################################################################
    else:
...
def batchnorm_backward(dout, cache):
    ...
    x = cache['x']
    mean = cache['mean']
    xminmean = cache['xminmean']
    var = cache['var']
    varsq = cache['varsq']
    divvarsq = cache['divvarsq']
    xhat = cache['xhat']
    gamma = cache['gamma']
    beta = cache['beta']
    eps = cache['eps']
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    for n in range(N):
        for f in range(F):
            for hi in range(int(H2)):
                for wi in range(int(W2)):
                    slice = x_pad[n, :, hi * stride:hi * stride + HH, wi * stride:wi * stride + WW]
                    out[n, f, hi, wi] = np.sum(slice * w[f, :, :, :]) + b[f]
        ...
        for f in range(F):
            for hi in range(int(H2)):
                for wi in range(int(W2)):
                    val = dout[n, f, hi, wi]
                    db[f] += val
                    dx_pad[n, :, hi * stride:hi * stride + HH, wi * stride:wi * stride + WW] += val * w[f, :, :, :]
                    dw[f, :, :, :] += val * x_pad[n, :, hi*stride:hi * stride + HH, wi * stride:wi * stride + WW]
